...
### 🧭 <mark>一个</mark>网页自动化向导
<mark>一个</mark>[ 网页自动化向导 ](#wizard-automation)，通过鼠标傻瓜式点击就能帮忙完成文稿、TTS、渲染、发布全流程。
...
2. **[方式1: 网页自动化向导](#wizard-automation)**（**推荐**）
   - 执行 `pnpm automation`，浏览器自动打开 **自动化向导**
   - 按提示完成文稿、TTS、渲染；
   - 输出在 `output/` 目录下。
3. **[方式2: CLI 命令行](#usage-guide)** : 与向导相同的 `pnpm` 命令，适合终端、脚本与进阶组合。
...
- **顺序：** **STEP1** → **STEP2** → **STEP3**（与网页向导相同；发布仍用下方 `login:*` / `upload:*`）
- **路径：** 与上文「输出文件」目录树一致
