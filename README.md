...
- **Node.js** >= 18.0.0
- **Python** 3.8 及以上（用于 TTS；仓库 CI 使用 3.11）
- **pnpm** >= 8.0.0（推荐）或 npm/yarn

#### Python 依赖（TTS）

语音与字幕生成依赖下列 **pip** 包，版本见 [`tts/requirements.txt`](tts/requirements.txt)：

| 包名 | 用途 |
|------|------|
| `edge-tts` | 基于 Microsoft Edge 的 TTS |
| `pydub` | 音频处理 |

安装 Node 依赖后，`package.json` 的 `postinstall` 会执行 `scripts/setup-python.sh`：在本机存在 `python3` 时创建虚拟环境 **`tts/venv`**，并用 `pip install -r tts/requirements.txt` 安装上述包。若未安装 Python，`pnpm install` 仍会成功；需要先安装 `python3`，再执行一次 `pnpm install` 或参考 [`tts/README.md`](tts/README.md) 手动创建 venv。
...
2. **安装依赖**
   ```bash
   pnpm install
   # 若已安装 python3：会在 tts/venv 中自动安装 Python（pip）依赖
   ```
...
# 微信视频号登录（Cookie 有效期较短，需定期重新登录；与公众号不同）
pnpm login:weixin-video

# 微信公众号后台登录（用于发文等自动化，与视频号不同）
pnpm login:weixin-mp
